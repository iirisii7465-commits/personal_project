...
public class MusicBoardDao {
    ...
    public int saveBoard(MusicBoardDto dto) {
        ...
        try {
            ...
            // 2. 게시글 저장
            sql =
                "insert into my_안지윤_music_board "
              + "(board_no, id, music_no, recommend_reason, board_content) "
              + "values (?, ?, ?, ?, ?)";
    ...
    public ArrayList<MusicBoardDto> getBoardList(
    		String select,
    		String search,
    		int start,
    		int end) {

    	ArrayList<MusicBoardDto> list =
    			new ArrayList<MusicBoardDto>();
    	...
    	String sql =
    		...
    		"               m.album_image, " +
    		"               (select count(*) " +
    		"                  from my_안지윤_music_like l " +
    		"                 where l.board_no = b.board_no) as like_count " +
    		"        from my_안지윤_music_board b " +
    		"        join my_안지윤_music m " +
    		"        on b.music_no = m.music_no " +
    		"        where " + select + " like ? " +
    ...
    public MusicBoardDto getBoardView(int no) {
    	 MusicBoardDto dto = null;
    	    ...
    	    String sql =
    	        "select b.*, m.title, m.artist, m.album_image " +
    	        "from my_안지윤_music_board b " +
    	        "join my_안지윤_music m " +
    	        "on b.music_no = m.music_no " +
    	        "where b.board_no = ?";
    ...
    public int updateBoard(MusicBoardDto dto) {
        ...
        String sql =
            "update my_안지윤_music_board " +
            "set recommend_reason = ?, " +
            "    board_content = ? " +
            "where board_no = ?";
